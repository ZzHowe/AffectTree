...
class DataProto:
    ...
    def repeat_uid(self, raw_uid_list , uid_list, target_key="uid", interleave: bool = True) -> "DataProto":
        count_dict = dict(Counter(uid_list))


        raw_batch_size = None
        for key_non in self.non_tensor_batch:
            raw_batch_size = len(self.non_tensor_batch[key_non])
            break
        
        repeated_tensors_list =[{}]*raw_batch_size
        repeated_non_tensor_batch_list = [{}]*raw_batch_size

        for i in range(self.batch.batch_size[0]):
            
            temp_uid = raw_uid_list[i]
            repeat_times = count_dict.get(temp_uid,1)
            if self.batch is not None:
                if interleave:  # interleave the data
                    repeated_tensors_list[i] = {
                        key: tensor[i].unsqueeze(0).repeat_interleave(repeat_times, dim=0) for key, tensor in self.batch.items()
                    }
                else:  # stack the data
                    repeated_tensors_list[i] = {
                        key: tensor[i].unsqueeze(0).unsqueeze(0).expand(repeat_times, *tensor.shape).reshape(-1, *tensor.shape[1:])
                        for key, tensor in self.batch.items()
                    }
            else:
                repeated_batch = None
            repeated_non_tensor_batch_list[i]={}
            for key, value in self.non_tensor_batch.items():
                if interleave:
                    repeated_non_tensor_batch_list[i][key] = np.repeat(np.expand_dims(value[i], axis=0), repeat_times, axis=0)
                else:
                    repeated_non_tensor_batch_list[i][key] = np.tile(np.expand_dims(value[i], axis=0), (repeat_times,) + (1,) * (value.ndim - 1))

        repeated_tensors = {}
        
        if self.batch is not None:
            if repeated_tensors_list and any(repeated_tensors_list):  
                all_keys = repeated_tensors_list[0].keys()
                

                for key in all_keys:
                    tensors_to_cat = []
                    for tensor_dict in repeated_tensors_list:
                        if tensor_dict and key in tensor_dict:
                            tensors_to_cat.append(tensor_dict[key])
                    if tensors_to_cat:
                        repeated_tensors[key] = torch.cat(tensors_to_cat, dim=0)
        ...
        if repeated_non_tensor_batch_list and any(repeated_non_tensor_batch_list): 
            all_keys = repeated_non_tensor_batch_list[0].keys()

            

            for key in all_keys:
                arrays_to_cat = []
                for batch_dict in repeated_non_tensor_batch_list:
                    if batch_dict and key in batch_dict:
                        arrays_to_cat.append(batch_dict[key])
                if arrays_to_cat:
                    repeated_non_tensor_batch[key] = np.concatenate(arrays_to_cat, axis=0)
